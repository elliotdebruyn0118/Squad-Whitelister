...
async function init() {
  ...
  let reconnectInt1 = null;
  let reconnectInt2 = null;
  console.log("SquadJS WebSocket");

  if (
    config.squadjs.websocket.token != "" &&
    config.squadjs.websocket.host1 != "" &&
    config.squadjs.websocket.host2 != ""
  ) {
    const tm = setTimeout(() => {
      console.error(
        " > Connection timed out. Check your SquadJS WebSocket configuration."
      );
      console.log(" > Proceeding without SquadJS WebSocket.");
      if (cb) cb();
    }, 10000);

    const resIP1 = (await lookup(config.squadjs.websocket.host1)).address;
    const resIP2 = (await lookup(config.squadjs.websocket.host2)).address;
    
    const socket1 = io(`ws://${resIP1}:${config.squadjs.websocket.port1}`, {
      auth: {
    ...
    const socket2 = io(`ws://${resIP2}:${config.squadjs.websocket.port2}`, {
      auth: {
        token: config.squadjs.websocket.token,
      },
      autoUnref: true,
    ...
    const handleConnect = (socket, reconnectInt) => {
      clearTimeout(tm);
    ...
    const handleDisconnect = (socket, reconnectInt) => {
      subcomponent_status.squadjs = false;
      console.log("SquadJS WebSocket\n > Disconnected\n > Trying to reconnect");
      reconnectInt = setInterval(() => {
        if (!subcomponent_status.squadjs) socket.connect();
      }, 10 * 1000);
    };

    socket1.on("connect", async () => {
      handleConnect(socket1, reconnectInt1);
    });
    socket1.on("disconnect", async () => {
      handleDisconnect(socket1, reconnectInt1);
    });
    socket1.on("PLAYER_CONNECTED", async (dt) => {
      try {
        if (dt && dt.player && dt.player.steamID) {
          mongoConn(async (dbo) => {
            dbo.collection("players").updateOne(
              { steamid64: dt.player.steamID },
              { $set: { username: dt.player.name } },
              { upsert: true }
            );
          });
          setTimeout(() => {
    ...
    socket2.on("connect", async () => {
      handleConnect(socket2, reconnectInt2);
    });
    socket2.on("disconnect", async () => {
      handleDisconnect(socket2, reconnectInt2);
    });
    socket2.on("PLAYER_CONNECTED", async (dt) => {
      try {
        if (dt && dt.player && dt.player.steamID) {
          mongoConn(async (dbo) => {
            dbo.collection("players").updateOne(
    ...
    subcomponent_data.socket1 = socket1;
    subcomponent_data.socket2 = socket2;
            // subcomponent_data.socket.on("PLAYER_DISCONNECTED", async (dt) => {
            //     console.log("Player disconnected: ", dt)
            // })
            subcomponent_data.socket1.on("CHAT_MESSAGE", async (dt) => {
                                            ...
                                            dbo.collection("players").updateOne({ discord_user_id: dbRes.discordUserId }, { $set: { steamid64: dt.player.steamID, username: dt.player.name, discord_user_id: dbRes.discordUserId, discord_username: discordUsername, ...oldPlayerData } }, { upsert: true }, (err, dbResU) => {
                                                dbo.collection("players").deleteOne({ steamid64: dt.player.steamID, discord_user_id: { $exists: false } }, (err, dbResRem) => {
                                                    ...
                                                    if (err) serverError(null, err);
                                                        ...
                                                        subcomponent_data.socket1.emit("rcon.warn", dt.steamID, "Linked Discord profile: " + discordUsername, (d) => { })
														subcomponent_data.socket2.emit("rcon.warn", dt.steamID, "Linked Discord profile: " + discordUsername, (d) => { })
                                                        discordUser.send({
                                                            embeds: [
                                                                new Discord.EmbedBuilder()
                                                                    .setColor(config.app_personalization.accent_color)
...
subcomponent_data.socket2.on("CHAT_MESSAGE", async (dt) => {
                switch (dt.message.toLowerCase().replace(/^(!|\/)/, '')) {
                    case 'test':
                        break;
                    case 'playerinfo':
                ...
                function _check() {
                    ...
                    if (subcomponent_status.squadjs) {
                        mongoConn(async dbo => {
                            ...
                            subcomponent_data.socket1.emit("rcon.getListPlayers", async (players) => {
                                if (players && players.length >= (stConf.seeding_start_player_count || 2) && players.length < stConf.seeding_player_threshold) {
  ...
  // Look for the match with the highest ID and "sever" column as "2"
  const query = 'SELECT layerClassname FROM DBLog_Matches WHERE server = 1 ORDER BY ID DESC LIMIT 1';
  connection.query(query, (error, results) => {
	if (error) {
	  throw error;
	}
const layerclasstf = results[0].layerClassname;
console.log(layerclasstf);
									
                                    if (
									  (players.length <= 40) &&
									  (layerclasstf.toLowerCase().includes('seed') || layerclasstf.toLowerCase().includes('seeding'))
									) {
                                        // console.log("current seeders", objArrToValArr(players, "name"));

                                        for (let p of players) {
                                            const oldPlayerData = dbo.collection("players").findOne({ steamid64: p.steamID });
                                                ...
                                                if (err) serverError(null, err)
                                                    ...
                                                    if (step > 0 && step > stepOld) {
                                                        if (percentageCompleted < 100) {
                                                            subcomponent_data.socket1.emit("rcon.warn", p.steamID, `Seeding Reward: \n\n${percentageCompleted}% completed`, (d) => { })
															subcomponent_data.socket2.emit("rcon.warn", p.steamID, `Seeding Reward: \n\n${percentageCompleted}% completed`, (d) => { })
                                                            ...
                                                            const reward_group = dbo.collection('groups').findOne({ _id: ObjectID(st.config.reward_group_id) })
                                                            let message =
                                                            ...
                                                            subcomponent_data.socket1.emit("rcon.warn", p.steamID, message, (d) => { })
															subcomponent_data.socket2.emit("rcon.warn", p.steamID, message, (d) => { })
                                                            if (subcomponent_status.discord_bot) {
                                                                const embeds = [
                                                                    new Discord.EmbedBuilder()
                                                                        .setColor(config.app_personalization.accent_color)
							...
							subcomponent_data.socket2.emit("rcon.getListPlayers", async (players) => {
                                if (players && players.length >= (stConf.seeding_start_player_count || 2) && players.length < stConf.seeding_player_threshold) {
                                    if (st.config.tracking_mode == 'incremental') {
                                        let deduction_points = 0;
  ...
  // Look for the match with the highest ID and "sever" column as "2"
  const query = 'SELECT layerClassname FROM DBLog_Matches WHERE server = 2 ORDER BY ID DESC LIMIT 1';
  connection.query(query, (error, results) => {
	if (error) {
	  throw error;
	}
                        ...
                        if (err) serverError(null, err);
                            ...
                            dbo.collection("players").findOne({ steamid64: dt.player.steamID }, async (err, dbResP) => {
                                if (err) serverError(null, err);
                                    ...
                                    if (subcomponent_status.squadjs) {
                                        setTimeout(() => {
                                            subcomponent_data.socket1.emit("rcon.warn", dt.player.steamID, msg, (d) => { })
											subcomponent_data.socket2.emit("rcon.warn", dt.player.steamID, msg, (d) => { })
                                            console.log(msg);
                                        }, timeoutDelay)
                                    }
                                }
